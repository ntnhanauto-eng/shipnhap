...
if (!empty($f_status)) {
    if ($f_status === 'Chờ duyệt') {
        // Bên ngoài người dùng xem: Trạng thái "Đã sơ duyệt" vẫn tính là "Chờ duyệt"
...
// Gom nhóm các mặt hàng theo cùng 1 request_code chuẩn van-phong-pham.php
$sqlList = "
    SELECT 
        p.id,
        p.request_code,
...
<style>
    ...
    .history-card { background: #ffffff; border: 2px solid #0284c7; border-radius: 12px; padding: 20px 24px; box-shadow: 0 4px 15px rgba(2,132,199,0.06); margin-bottom: 30px; }
    ...
    /* Modal xem mặt hàng & modal chọn mặt hàng */
...
<div class="prop-form-wrapper">
    ...
    <!-- KHỐI 1: DANH SÁCH ĐỀ XUẤT VĂN PHÒNG PHẨM (ĐỒNG BỘ VỚI VAN-PHONG-PHAM.PHP) -->
    <div class="history-card">
        <div class="history-title">
            <span>📬 DANH SÁCH ĐỀ XUẤT VĂN PHÒNG PHẨM (Tổng: <?= $total_rows ?> đợt yêu cầu)</span>
            <span style="font-size: 0.82rem; font-weight: normal; color: #64748b;">Dữ liệu đồng bộ trực tiếp từ hệ thống quản lý VPP</span>
        </div>
                ...
                <tbody>
                    <?php if (!empty($proposalsList)): ?>
                        ...
                        foreach ($proposalsList as $p): 
                            ...
                            // Xử lý hiển thị trạng thái (Ẩn Sơ duyệt, coi như Chờ duyệt)
    ...
    <!-- KHỐI 2: FORM ĐỀ XUẤT CẤP VĂN PHÒNG PHẨM MỚI -->
    <div class="form-card">
...
<!-- ==================== MODAL XEM CHI TIẾT CÁC MẶT HÀNG (ĐỒNG BỘ VỚI VAN-PHONG-PHAM.PHP) ==================== -->
...
// Bộ nhớ đệm tạm thời cho các mặt hàng đã chọn
let selectedItems = {};
...
// BẬT POPUP XEM CHI TIẾT MẶT HÀNG (ĐỒNG BỘ VAN-PHONG-PHAM.PHP)
