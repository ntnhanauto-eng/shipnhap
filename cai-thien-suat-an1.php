...
try {
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS `meal_menus` (
            ...
            `week_code` VARCHAR(20) NOT NULL, -- VD: 2026-W37
            `year` INT NOT NULL,
            `day_of_week` VARCHAR(20) NOT NULL, -- Thứ 2, Thứ 3...
            `meal_type` VARCHAR(30) DEFAULT 'Bữa Trưa', -- Sáng, Trưa, Tối
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS `meal_checks` (
            ...
            `is_correct` TINYINT(1) DEFAULT 1, -- 1: Đúng, 0: Không đúng
            `adjustment_percent` INT DEFAULT 0, -- % điều chỉnh
            `quality_rating` VARCHAR(50) DEFAULT 'Tốt', -- Tốt, Trung bình, Kém
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // 9. Trích xuất thông minh từ File Excel / Hình ảnh
...
// Lấy danh sách mã tuần có trong hệ thống để đưa vào bộ lọc
$availableWeeks = [];
try {
...
// Lấy danh sách thực đơn theo tuần được chọn để kiểm tra hàng ngày
$todayMenus = [];
try {
    $stmtM = $pdo->prepare("
...
// Bộ lọc thời gian cho thống kê (1 tuần, 2 tuần, 3 tuần, tháng, năm)
...
<div class="meal-wrapper">
    ...
    <form method="GET" action="cai-thien-suat-an.php" class="card-box" style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; padding:15px 20px;">
        ...
        <?php if ($selected_week !== $current_week): ?>
            <a href="cai-thien-suat-an.php" style="font-size:0.85rem; color:#0284c7; text-decoration:none;">Xem tuần hiện tại</a>
        <?php endif; ?>
    </form>
                ...
                <tbody>
                    <?php if (!empty($todayMenus)): ?>
                        <?php foreach ($todayMenus as $m): ?>
                            <form method="POST" action="cai-thien-suat-an.php?f_week=<?= urlencode($selected_week) ?>">
    ...
    <!-- BỘ LỌC THỜI GIAN THỐNG KÊ (3, 4, 6) -->
    <form method="GET" action="cai-thien-suat-an.php" class="card-box" style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; padding:15px 20px;">
